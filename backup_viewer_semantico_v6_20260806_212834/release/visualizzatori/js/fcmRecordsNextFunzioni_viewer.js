(function (global, doc) {
  ...
  function directViews(data) {
    ...
    order.forEach(function (key, index) {
      ...
      if (rows.length) views.push({ id: key, label: labels[key] || C.italianLabel(key), rows: rows, order: index });
    });
    return views;
  }
  ...
  function preferredColumns(rows) {
    var priority = [
      'position','rank','stagione','seasonId','competizioneNome','competitionName','competizioneId','competizione','girone',
  ...
  function renderCell(row, key) {
    ...
    if (key === 'eventType' || key === 'direction' || key === 'recordId' || key === 'status') {
  ...
  function renderTable(rows) {
    ...
    columns.forEach(function (column) {
      ...
      html += '<th data-sort="' + C.escapeHtml(column) + '">' + C.escapeHtml(C.italianLabel(column) + marker) + '</th>';
    });
    html += '</tr></thead><tbody>';
  ...
  function renderStatus() {
    ...
    host.innerHTML = statuses.map(function (item) {
      ...
      return '<div class="rn-status-row ' + cls + '"><strong>' + C.escapeHtml(C.italianLabel(item.childId || status)) + '</strong><span>' + C.escapeHtml(C.italianLabel(item.detail || item.message || status)) + '</span></div>';
    }).join('');
  }
